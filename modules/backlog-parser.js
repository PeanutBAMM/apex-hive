...
function estimateEffort(title, description) {
  ...
  if (text.includes('system') || text.includes('infrastructure') || 
      text.includes('refactor') || text.includes('migration')) {
    return 8;
  }
  
  // Keywords that indicate medium effort
  if (text.includes('implement') || text.includes('create') || 
      text.includes('build') || text.includes('enhance')) {
    return 5;
  }
  
  // Keywords that indicate low effort
  if (text.includes('fix') || text.includes('update') || 
      text.includes('add') || text.includes('small')) {
    return 3;
  }
...
function estimateValue(title, description, priority) {
  ...
  if (text.includes('performance') || text.includes('security') || 
      text.includes('critical') || text.includes('user')) {
    baseValue += 2;
  }
...
function estimateRisk(title, description) {
  ...
  if (text.includes('breaking') || text.includes('migration') || 
      text.includes('security') || text.includes('critical')) {
    return 4;
  }
  
  if (text.includes('refactor') || text.includes('update') || 
...
export async function loadBacklogItems() {
  try {
    ...
    if (items.length > 0) {
      console.error(`[BACKLOG-PARSER] Loaded ${items.length} items from BACKLOG.md`);
      return items;
    }
  ...
  return [
    {
      ...
      description: 'This is sample data. Create a BACKLOG.md file to see real items.',
      priority: 'high',
      status: 'pending',
      category: 'sample',
      effort: 1,
