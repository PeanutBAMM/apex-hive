...
async function getLatestConversation() {
  try {
    ...
    const summaryMatch = content.match(/## Conversation Summary\n\n([\s\S]*?)(?=\n##|$)/);
    let fullSummary = 'No summary available';
    
    if (summaryMatch) {
      // Extract key sections from summary
      const accomplishmentsMatch = summaryMatch[1].match(/### What We Accomplished\n\n([\s\S]*?)(?=\n###|$)/);
      const scopeMatch = summaryMatch[1].match(/### Scope of Changes\n\n([\s\S]*?)(?=\n###|$)/);
      const challengesMatch = summaryMatch[1].match(/### Challenges & Solutions\n\n([\s\S]*?)(?=\n###|$)/);
      
      fullSummary = [];
      if (accomplishmentsMatch) fullSummary.push(accomplishmentsMatch[1].trim());
    ...
    return {
      ...
      tags: tagsMatch ? tagsMatch[1].split('`, `').map(t => t.replace(/`/g, '')) : [],
...
async function getGitContext() {
  try {
    ...
    const recentCommits = execSync('git log --oneline -3', { encoding: 'utf8' }).trim().split('\n');
    ...
    try {
      ...
      const ahead = execSync('git rev-list --count @{u}..HEAD', { encoding: 'utf8' }).trim();
...
async function getBacklogItems() {
  try {
    ...
    const highPriorityMatch = content.match(/## 🔥 High Priority\n\n([\s\S]*?)(?=\n##|$)/);
    const items = [];
...
export async function run(args = {}) {
  ...
  try {
    ...
    const [conversation, git, cache, issues, backlog, migration] = await Promise.all([
    ...
    if (conversation && migration && conversation.tags.includes('migration')) {
      focusAreas.push(`Continue cached file operations migration (${migration.percentage}% complete)`);
    ...
    if (git && git.changeCount > 5) {
      focusAreas.push(`Review and commit ${git.changeCount} uncommitted changes`);
    }
