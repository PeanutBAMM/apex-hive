...
export async function run(args = {}) {
  ...
  try {
    ...
    if (verbose) {
      console.error(
        `[CACHE-WARM-CONVERSATIONS] Current cache: ${statsBefore.items} items, ${formatBytes(statsBefore.totalSize)}`,
      );
    }

    // Read conversation index from disk
    const conversations = await readConversationsFromDisk(
      directory,
      limit,
      daysBack,
    );
    ...
    for (const conv of conversations) {
      try {
        ...
        if (existing) {
          ...
          if (verbose) {
            console.error(
              `[CACHE-WARM-CONVERSATIONS] Already cached: ${conv.title}`,
            );
          }
        } else if (!dryRun) {
          // Read full conversation content
          const content = await fs.readFile(
            path.join(directory, conv.file),
            "utf8",
          );
          ...
          const cacheEntry = {
            ...
            wordCount: narrativeSummary
              ? narrativeSummary.split(/\s+/).length
              : 0,
        ...
        console.error(
          `[CACHE-WARM-CONVERSATIONS] Error processing ${conv.title}:`,
          error.message,
        );
      }
    }
...
async function readConversationsFromDisk(directory, limit, daysBack) {
  try {
    ...
    console.error(
      "[CACHE-WARM-CONVERSATIONS] Error reading index:",
      error.message,
    );
    ...
    try {
      ...
      for (const file of mdFiles) {
        try {
          ...
          console.error(
            `[CACHE-WARM-CONVERSATIONS] Error reading ${file}:`,
            err.message,
          );
        }
      }
      ...
      console.error(
        "[CACHE-WARM-CONVERSATIONS] Fallback error:",
...
function extractNarrativeSummary(content) {
  ...
  const anySummaryMatch = content.match(
    /### Summary[\s\S]*?(?=\n## |\n### |$)/,
  );
  if (anySummaryMatch) {
  ...
  const contentLines = lines
    .slice(contentStart, contentStart + 20)
